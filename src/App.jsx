...
const WEAPONS = { CANNON: { id: 'CANNON', name: 'CANNON', type: 'SINGLE', damage: 40, speed: 255, color: 0xffff00, radius: 6 }, FLAK: { id: 'FLAK', name: 'FLAK', type: 'SPREAD', damage: 15, speed: 340, color: 0xff4400, radius: 4 } };
...
export default function App() {
    ...
    const startTraining = () => { setIsTraining(true); setIsHost(true); setRoomId('OFFLINE'); setGameState('MENU'); setStatusMsg(t('STATUS_P2_CONNECTED')); };
    const createRoom = async () => {
        setIsTraining(false); setStatusMsg(t('STATUS_CREATING'));
        try {
            const user = await signInAnonymously(auth);
            const uid = user.user.uid; setIsHost(true);
            const code = Math.random().toString(36).substring(2, 6).toUpperCase(); setRoomId(code);
    ...
    const joinRoom = async () => {
        ...
        setIsTraining(false); setStatusMsg(t('STATUS_JOINING'));
        try {
            const user = await signInAnonymously(auth);
            const uid = user.user.uid; setIsHost(false);
    ...
    const restartGame = () => { setRunId(prev => prev + 1); setGameState('PLAYING'); };
    const backToMenu = () => { setMySquad([]); setGameState('LOBBY'); window.location.reload(); };
                    ...
                    {gameState === 'PLAYING' && (
                        <>
                            <div style={{position: 'absolute', top: 50, right: 20, zIndex: 1000}}>
                                <button style={{background: 'rgba(255, 0, 0, 0.2)', border: '1px solid #ff0000', color: '#ff0000', padding: '5px 10px', borderRadius: 4, cursor: 'pointer', fontWeight: 'bold', fontSize: '10px'}} onClick={() => { if (window.confirm(t('SURRENDER_CONFIRM'))) { if (!isTraining) { const surrenderRef = ref(db, `rooms/${roomId}/surrender`); set(surrenderRef, isHost ? 'HOST' : 'GUEST'); } else { handleGameOver("DEFEAT"); } } }}>{t('BTN_SURRENDER')}</button>
                            </div>
