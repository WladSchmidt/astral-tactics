...
const SHIP_STATS = {
    FLUX: { 
        id: 'FLUX', name: 'FLUX', descKey: 'DESC_FLUX',
        hp: 90, speed: 155, color: 0x00ffff, radius: 24, moveRange: 385, sprite: 'flux_img',
        hitRadius: 150, hitOffset: { x: 170, y: 200 }
    },
    VECTOR: { 
        id: 'VECTOR', name: 'VECTOR', descKey: 'DESC_VECTOR',
        hp: 120, speed: 110, color: 0x00ff00, radius: 28, moveRange: 310, sprite: 'vector_img',
        hitRadius: 150, hitOffset: { x: 140, y: 180 }
    },
    COLOSSUS: { 
        id: 'COLOSSUS', name: 'COLOSSUS', descKey: 'DESC_COLOSSUS',
        hp: 180, speed: 75, color: 0xffaa00, radius: 38, moveRange: 220, sprite: 'colossus_img',
        hitRadius: 155, hitOffset: { x: 150, y: 180 }
    }
};
...
export default function App() {
    ...
    const playClick = () => {
        ...
        audio.volume = 0.5;
        audio.play().catch(e => { /* Ignora erro se usuário não interagiu ainda */ });
    };
    ...
    const createRoom = async () => {
        ...
        try {
            ...
            onValue(ref(db, `rooms/${code}`), (snapshot) => {
                ...
                if (data && data.guest) { setStatusMsg(t('STATUS_P2_CONNECTED')); setGameState('MENU'); }
            });
    ...
    const joinRoom = async () => {
        ...
        try {
            ...
            if (snap.exists()) { await update(rRef, { guest: uid }); setRoomId(joinCode); setGameState('MENU'); } 
            else { setStatusMsg(t('STATUS_NOT_FOUND')); }
        } catch (e) { setStatusMsg(t('STATUS_ERROR') + e.message); }
    };
    ...
    const lockInSquad = async () => {
        ...
        onValue(ref(db, `rooms/${roomId}`), (snapshot) => {
            const data = snapshot.val();
            if (data && data.hostSquad && data.guestSquad) { setGameState('PLAYING'); }
        });
    };
...
const PhaserGame = ({ roomId, isHost, isTraining, mySquadList, onGameOver, onExit, lang, t }) => {
    ...
    useEffect(() => {
        ...
        let soundBtn = null; let isMuted = false;

        // 🔥 SONS (DECLARAÇÃO)
        let sfxShoot, sfxExplosion, sfxClick, bgmMusic;
        ...
        async function create() {
            ...
            // 🎵 CONFIGURAR SONS
            sfxShoot = scene.sound.add('shoot', { volume: 0.4 });
            sfxExplosion = scene.sound.add('explosion', { volume: 0.5 });
            sfxClick = scene.sound.add('click', { volume: 0.5 });
            bgmMusic = scene.sound.add('bgm', { volume: 0.3, loop: true });
            
            if (!scene.sound.locked) bgmMusic.play();
            else scene.sound.once(Phaser.Sound.Events.UNLOCKED, () => bgmMusic.play());
            ...
            // 🎵 BOTÃO DE MUTE
            soundBtn = scene.add.text(TOTAL_WIDTH - 80, 20, t('MUTE'), { font: '12px Arial', fill: '#888', backgroundColor: '#222', padding: 5 }).setInteractive().setDepth(1000);
            soundBtn.on('pointerdown', () => {
                isMuted = !isMuted; scene.sound.mute = isMuted;
                soundBtn.setText(isMuted ? t('UNMUTE') : t('MUTE'));
                soundBtn.setColor(isMuted ? '#ff0000' : '#888');
            });
            ...
            if (isTraining) mapData = generateMapData();
                ...
                try {
                    ...
                    if (roomData && roomData.map) mapData = roomData.map;
                    else mapData = generateMapData();
            ...
            mySquadData.forEach((id, i) => spawnShip(scene, 262 + (i*250), pY, SHIP_STATS[id], 'PLAYER', i));
            enemySquadData.forEach((id, i) => spawnShip(scene, 262 + (i*250), eY, SHIP_STATS[id], 'ENEMY', i));
            ...
            if (!isTraining) {
                ...
                unsubscribeSurrender = onValue(surrenderRef, (snapshot) => {
                    ...
                    if (whoSurrendered) {
                        // ✅ CORREÇÃO TELA PRETA: NÃO DESTRUA O JOGO AQUI.
                        // O React vai desmontar o componente e limpar tudo sozinho.
                        const myRole = isHost ? 'HOST' : 'GUEST';
                        if (whoSurrendered === myRole) onGameOver('DEFEAT'); else onGameOver('VICTORY');
                    }
                });
            }

            scene.physics.add.overlap(projectileGroup, shipsGroup, (projectile, ship) => {
                if (projectile.owner === ship || projectile.owner.faction === ship.faction) return;
                takeDamage(scene, ship, projectile.damage);
            ...
            createButton(scene, TOTAL_WIDTH - 120, centerY, t('BTN_EXECUTE'), 160, 60, 0x008800, () => { if(!isExecuting) { sfxClick.play(); submitTurn(scene); }}, staticHudGroup);
        ...
        function setupInputs(scene) {
            scene.input.on('pointerdown', (pointer) => {
                ...
                if (clickedShip) {
                    ...
                    if (selectedShip) selectedShip.isSelected = false; sfxClick.play();
                ...
                if (selectedShip && selectedShip.active) {
                    ...
                    if (pointer.rightButtonDown()) { sfxClick.play(); selectedShip.plannedAttack = { x: target.x, y: target.y }; updateHandles(); drawUI(scene); } 
                        ...
                        if (dist <= selectedShip.stats.moveRange) {
                            if(checkRaycast(selectedShip.x, selectedShip.y, target.x, target.y)) { sfxClick.play(); selectedShip.plannedMove = { x: target.x, y: target.y }; updateHandles(); drawUI(scene); } 
        ...
        function drawHP(ship) {
            const pct = Math.max(0, ship.hp / ship.stats.hp);
            graphics.fillStyle(0x000000); graphics.fillRect(ship.x - 20, ship.y - 45, 40, 6);
            graphics.fillStyle(ship.faction === 'PLAYER' ? 0x00ff00 : 0xff0000); graphics.fillRect(ship.x - 20, ship.y - 45, 40 * pct, 6);
        }
        ...
        return () => { 
            ...
            // ✅ CORREÇÃO DO AUDIO ERROR: Limpar sons antes de destruir
            if(gameRef.current) {
                try {
                ...
                } catch(e) { console.log("Audio cleanup safe", e); }
